...
function renderLocs(locs) {
  ...
  var strHTML = locs
    .map((loc) => {
      ...
      let latLng
      if (gUserPos) {
        latLng = { lat: loc.geo.lat, lng: loc.geo.lng }
      ...
      return `
        <li class="loc ${className}" data-id="${loc.id}">
            <h4>  
                <span>${loc.name}</span>
                <span>${dist ? `Distance:${dist}Km` : ''}</span>
...
function displayLoc(loc) {
  ...
  el.querySelector('.loc-dist').innerText = dist ? `Distance:${dist}Km` : ''
